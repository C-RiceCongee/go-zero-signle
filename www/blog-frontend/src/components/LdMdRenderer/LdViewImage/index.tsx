...
const RenderPreview: React.FC<IRenderPreview> = memo(({ url, setUrl }) => {
  ...
  return (
    <div
      ref={ref}
      className="h-full w-full fixed left-0 top-0 z-50 flex items-center justify-center  bg-skin-menu-bg backdrop-blur drop-shadow-sm"
    >
      <img
        ref={imgRef}
        draggable={true}
        className={`w-4/5 duration-100 origin-cente`}
        style={{ transform: `scale(${scale})` }}
        src={url}
      ></img>
      <div className="bottomAction fixed bottom-10 flex items-center justify-center">
        <div className="scalePercent">缩放比例:{Math.floor(scale * 100)}%</div>
        <div className="exit cursor-pointer" onClick={onExit}>
          退出
        </div>
      </div>
    </div>
